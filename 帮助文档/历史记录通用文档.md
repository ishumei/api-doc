...
| description | 数组 | 否 | 风险原因，不传为全部，["涉政：一号领导：一号领导"] |
| search | json对象 | 否 | 组合查询，具体看下方示例|
| scroll | boolean | 否 | 是否启用滚动查询 |
| lastTimeStamp | int | 否 | 上一次查询结果中最后一条数据的时间戳 |
| _id | string | 否 | 上一次查询结果中最后一条数据的文档 ID |
...
＂type":＂涉政识别电商BUILTIN＂,
"lastTimeStamp":1708399329000,
"lastId":"xrqILpwBXj_uIB7CLfxE"
