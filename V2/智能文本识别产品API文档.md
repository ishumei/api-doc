...
| type | string | 检测的风险类型或者场景 | Y | 可选值：<br/>`ZHIBO`：直播<br/>`ECOM`：电商<br/>`GAME`：游戏<br/>`NEWS`：新闻资讯<br/>`FORUM`：论坛<br/>`SOCIAL`：社交<br/>`QQ`：QQ<br/>`NOVEL`：小说<br/>`TEXTRISK`：默认值（包含：<br/>涉政、暴恐、违禁、色情、辱骂、广告、隐私、广告法、无意义）<br/>`FRUAD`：网络诈骗<br/>`UNPOACH`：高价值用户防挖<br/>`TEXTMINOR`：未成年人<br/>`PROMPTATTACK`：攻击指令<br/>以上type可以下划线组合，如：`ZHIBO_TEXTRISK_FRUAD` |
...
| nickname | string | 用户昵称，强烈建议传递此参数，几乎所有平台的恶意用户都会通过昵称散播垃圾信息，存在色情违禁和导流信息等风险，长度限制150字符，超出部分会被截断 | N |  |
