...
| **请求参数名** | **类型** | **参数说明** | **是否必传** | **规范** |
| --- | --- | --- | --- | --- |
| text | string | 需要检测的文本 | Y | 文本字数上限1万字，超过1万字只截取前1万字进行识别<br/>若传递nickname字段，则会检测text字段而不检测nickname字段。 |
...
| nickname | string | 用户昵称，强烈建议传递此参数，几乎所有平台的恶意用户都会通过昵称散播垃圾信息，存在涉政违禁和导流信息等风险 | N | 校验昵称内容风险<br/> 可检测的长度是150字符，超过会不检测风险|
