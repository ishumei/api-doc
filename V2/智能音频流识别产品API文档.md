...
| **请求参数名** | **类型**    | **是否必传** | **规范**                                                     |
| -------------- | ----------- | ------------ | ------------------------------------------------------------ |
| passThrough    | json_object | N            | 透传字段，该字段下所有字段会通过回调结果中 `detail["requestParams"]["extra"]["passThrough"]`返回。 |
...
| requestParams     | json_object | Y            | 返回请求参数data中的所有字段，`requestParams["extra"]["passThrough"]`为请求参数`data["extra"]["passThrough"]`中所有字段。                                                                                                                                                                                                           |
