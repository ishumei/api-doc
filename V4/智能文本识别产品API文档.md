...
| **请求参数名** | **类型** | **参数说明** | **是否必传** | **规范** |
| --- | --- | --- | --- | --- |
| text | string | 需要检测的文本 | Y | 当不传入lang字段；lang字段传入zh或lang字段传入auto且识别为中文时，字符数上限1万字，超过1万字符时会报错<br/>当lang字段传入的值不是zh或传入auto识别结果为非中文时，没有字符数上限<br/>若传递nickname字段，则会同时校验文本+昵称内容。|
