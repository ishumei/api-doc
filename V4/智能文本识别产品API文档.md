...
| **请求参数名** | **类型** | **参数说明** | **是否必传** | **规范** |
| --- | --- | --- | --- | --- |
| text | string | 需要检测的文本 | Y | 单次请求字符数上限1万字，超过1万字符时会报错。<br/>若传递nickname字段，则会检测text字段而不检测nickname字段。|
...
| nickname | string | 用户昵称 | N | 校验昵称内容风险<br/> 可检测的长度是150字符，超过会不检测风险|
...
| --- | --- | --- | --- | --- |
| code | int| 返回码 | Y | `1100`：成功<br/>`1901`：QPS超限<br/>`1902`：参数不合法<br/>`1903`：服务失败<br/>`1905`:单次请求字符超过10000<br/>`9100`：余额不足<br/>`9101`：无权限操作 |
