...
| userId            | int      | 用户账号标识 | N           | AGORA、TRTC、GIN分流情况下存在，返回的userId是实际房间中的用户id，与请求参数中的uid无关。 |
| strUserId         | string   | 用户账号标识 | N           | TRTC、ZEGO、BYTE、GIN分流情况下存在。返回的userId是实际房间中的用户id，与请求参数中的uid无关。 |
| room              | string   | 房间号           | N           |                                                                                                 |
