...
| 集群 | URL | 支持产品列表 |
| --- | --- | --- |
| 北京 | `http://api-text-sf-bj.fengkongcloud.com/shunfeng/text/v4` | 中文文本 |
...
| **请求参数名** | **类型** | **参数说明** | **是否必传** | **规范** |
| --- | --- | --- | --- | --- |
| text | string | 需要检测的文本 | N | text、receiverName、company、address任意传入其一 |
| receiverName | string | 收件人姓名 | N | text、receiverName、company、address任意传入其一 |
| company | string | 公司 | N | text、receiverName、company、address任意传入其一 |
| address | string | 地址 | N | text、receiverName、company、address任意传入其一 |
| eventIds | map | text、receiverName、company、address、对应的eventId | Y | eventIds的传入的key需要和传入的text、receiverName、company、address对应，value传入对一下的eventId，联系数美开通。特殊情况当同时传入receiverName和address，receiverName和address会合并审核，还需要传入merged对应的eventId |
...
| riskLevel       | string   | 整体风险等级   | N            | PASS/REVIEW/REJECT                                                                           |
| isCircuitBroken | string   | 是否熔断     | N            | 仅在同级 riskLevel 字段为 REJECT 时下发<br/>true: 熔断<br/>false: 未熔断                                    |
| detail          | array    |          |              |                                                                                              |
...
| --- | --- | --- | --- | --- |
| requestId | string | 请求标识 | Y | 本次请求数据的唯一标识，用于问题排查和效果优化，强烈建议保存|
| identifier | string | 数据标识 | Y | receiverName、company、address、merged |
...
| **参数名称**| **类型** | **参数说明** | **是否必返** | **规范** |
| --- | --- | --- | --- | --- |
| qlabel| string | 问题标签| Y| 可选值：<br/>`UNKNOWN`：没有匹配<br/>`CANNOT_ASK`：问题本身不可提问/不可输入<br/>`EXACTNESS`：问题答案必须正确。包括立场正确<br/>`POSITIVE`：问题答案需要包含正向引导<br/>`PENDING`:流式查询代答答案<br/> |
| answer | string | 建议答案 | Y | 当qlabel为"EXACTNESS"或者"POSITIVE"时，会给出数美建议的符合要求的答案。|
| isEnd | bool | 答案是否完整返回 | Y |答案是否完整返回 |
| hasAnswer | int | 是否有答案 | Y |1 有代答；0 没有代答；2 需流式查询代答，默认为0 |
...
```json
{
    "code": 1100,
    "message": "success",
    "requestId": "dc484cb836a36d0e614612fecddc5e46",
    "riskLevel": "PASS",
    "detail": [
        {
            "allLabels": [],
            ...
            "identifier": "address",
            "langResult": {
                "translatedText": "nickname"
            },
            "requestId": "b96564a025a11a381a9ea3139db828d1",
            "resultType": 0,
        ...
        },
        {
            ...
            "requestId": "893e01f4b124f74a2b8d8d1e9b3ec281",
            "resultType": 0,
            "riskDescription": "normal",
            "riskDetail": {},
            "riskLabel1": "normal",
