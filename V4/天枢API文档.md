...
| roundId        | string      | 对话轮次标识     | 非必传参数   | 由数字、字母、下划线、短杠组成的长度小于等于32位的字符串     |
| role           | string      | 用户角色         | 必传参数     | 角色<br/>枚举值：<br/>user：用户<br/>assistant：机器人       |
| lang           | string      | 语言标识         | 非必传参数   | 当前仅支持传 `auto`                                          |
| userEnd        | bool        | 用户输入结束标志 | 非必传参数   | 默认值为false，当user输出结束，设置为true                    |
...
| **请求参数名** | **类型**    | **参数说明** | **是否必传** | **规范**                                        |
| -------------- | ----------- | ------------ | ------------ | ----------------------------------------------- |
| tokenId | string | tokenId | 非必传参数 | 写⼊历史记录⽤于历史记录数据检索 |
| userId    | string | ⽤户标识     | 非必传参数   | 写⼊历史记录⽤于历史记录数据检索 |
