...
| memberId	      | string	     | 会员ID，代表品牌内唯一的会员ID，跨渠道通用（天猫、京东等）	                                             | 必传参数     |                                                                                                                                                                                                                                                  |
| unionId	       | string	     | 微信唯一标识	                                                                      | 必传参数     |                                                                                                                                                                                                                                                  |
| ip             | string      | 当前业务事件发生时的客户端公网ipv4或ipv6地址                                                   | 必传参数     | 非内网ip                                                                                                                                                                                                                                            |
| timestamp      | int64       | 当前业务事件发生时的时间戳，单位为毫秒（ms）                                                      | 必传参数     |                                                                                                                                                                                                                                                  |
| deviceId       | string      | 数美设备指纹标识，由数美SDK生成                                                            | 非必传参数    | 数美设备指纹标识，用于用户行为分析。<br/>注：<br/>1. 接入数美SDK前的版本，无法获取到deviceId的情况下，deviceId传空“”即可；                                                          <br/>     2. 直接传输getDeviceId获取到的原始字段，不需要解密                                                               |
| os             | string      | 应用端操作系统类型                                                                    | 非必传参数     | 可选值：`android、harmony、ios、weapp、web、aliapp：支付宝小程序、ttapp：抖音小程序、tmapp：天猫小程序、mcp：通过agent调用`                                                                                                                                                                   |
| appVersion     | string      | 应用版本号                                                                        | 非必传参数     | 传入当前应用或服务正在使用的版本号，格式为3个点4段数字，每段数字最多4位，例如`3.2.1.1234`。不足4段的请补0，例如`2.1.5`传入`2.1.5.0`。多于4段的取前4段，例如`2.1.5.1.1`传入`2.1.5.1`。                                                                                                                           |
| activityId     | string      | 营销活动id                                                                       | 非必传参数     | 建议传入对应营销活动标识id                                                                                                                                                                                                                                   |
...
| 新零售    | 通用             | 取消预约             | cancelBooking         | [参数详情](#cancelBooking)         |
...
| bookId      | string | 预约唯一Id | 非必传参数     | 可以传入当前预约动作的唯一标识，如同订单号，可追溯对应原始预约id             |
| reserveTimestamp      | int64 | 预约时间对应的时间戳 | 非必传参数	     | 毫秒级别13位时间戳格式             |
...
#### <span id="cancelBooking"> 取消预约 </span>

| **参数名称**      | **类型**     | **参数说明**     | **是否必传** | **规范**        |
|---------------|------------|--------------|----------|---------------|
| [基础参数](#data) |            |              |          | 需要满足基础参数的配置要求 |
| bookId      | string | 预约唯一Id | 非必传参数     | 可以传入当前预约动作的唯一标识，如同订单号，可追溯对应原始预约id             |
| cancelType      | string | 取消类型 | 必传参数     | 可选值：<br/> `timeout`：超时取消<br/> `manuel`：手动取消          |
| reserveTimestamp      | int64 | 预约时间对应的时间戳 | 非必传参数	     | 毫秒级别13位时间戳格式             |
...
|--------------------------|-------------|--------------------------|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [基础参数](#data)            |             |                          |             | 需要满足基础参数的配置要求                                                                                                                                                                                                 |
| registerBaId             | string      | 会员卡开卡柜员ID                | 可选参数	       |                                                                                                                                                                                                               |
| registerCounterId	       | string      | 会员卡开卡柜台ID                | 可选参数	       |                                                                                                                                                                                                               |
| registerChannel	         | string      | 会员卡开卡渠道	                 | 可选参数        | 可选值：`Counter`、`TMall`、`JDMall`、`Member-Center`                                                                                                                                                                |
| products                 | json_array  | 订单商品信息，详情见下文             | 必传参数        |                                                                                                                                                                                                               |
| orderId                  | string      | 订单ID                     | 强烈建议        | 该商品订单唯一标识                                                                                                                                                                                                     |
| orderType                | string      | 订单类型                     | 必传参数(枚举值校验) | 可选值：<br/>`normal`：正装订单<br/>`trialSample`：试用小样订单<br/>`trialBox`：试用礼盒订单<br/>`exchangePoint`：积分兑礼订单<br/>`exchangeCoupon`：兑换券订单<br/>`counterSample`：到柜领样订单<br/>`machineSample`：派样机订单<br/>`purchaseCoupon`：优惠券购买订单 |
| tokenBirthday            | string      | 生日年月日                    | 可选参数        | yyyy-MM-DD，可以精确到日的话就精确到日，只能精确到月的话就默认当月第一天                                                                                                                                                                     |
| paymentId                | string      | 付款ID                     | 强烈建议        | 用户进行一次下单付款动作的标识；该标识标记了用户发起的一次下单付款行为。                                                                                                                                                                          |
| orderNotes               | string      | 订单备注信息                   | 建议          | 用户在此订单上备注的信息                                                                                                                                                                                                  |
| paymentMethod            | string      | 付款方式                     | 强烈建议        | 可选值：<br/>`CASH_ON_DELIVERY`：货到付款<br/>`ONLINE_PAYMENT`：在线支付                                                                                                                                                    |
| interval                 | int64       | 商品发布到下单时间，单位毫秒（ms）       | 强烈建议        | 主要用于商品秒杀                                                                                                                                                                                                      |
| bindPhone                | string      | 微信绑定手机号                  | 	可选参数       | 	MD5加密字符串格式为32位小写                                                                                                                                                                                             |
| bindPhoneMd5             | string      | 微信绑定手机号的phoneMd5加密串      | 	可选参数	      |                                                                                                                                                                                                               |
| bindPhoneSha256          | string      | 微信绑定手机号的Sha256加密串        | 	可选参数	      |                                                                                                                                                                                                               |
| bindPhoneCountryCode     | string      | 微信绑定手机号的国家代码             | 	可选参数       | 中国大陆区手机号填写0086非中国大陆区手机号填写国家代码例如：美国填写0001、安道尔0376、巴哈马1242                                                                                                                                                      |
| phone                    | string      | 用户下单使用的手机号或者该手机号MD5加密字符串 | 强烈建议        | 中国大陆区手机号请填写11位数字例如：13800138000MD5加密字符串格式为32位小写                                                                                                                                                                |
| receiverPhone            | string      | 收货人手机号                   | 强烈建议        | 中国大陆区手机号请填写11位数字例如：13800138000                                                                                                                                                                                |
| receiverPhoneMd5         | string      | 收货人手机号Md5值               | 强烈建议        | MD5加密字符串格式为32位小写例如：`d0213fde1643000949161dcb1ed0e605`                                                                                                                                                         |
| receiverPhoneSha256      | string      | 收货人手机号的Sha256加密串	        | 可选参数	       |                                                                                                                                                                                                               |
| receiverPhoneCountryCode | string	     | 收货人手机号的国家代码              | 	必传参数       | 中国大陆区手机号填写0086非中国大陆区手机号填写国家代码例如：美国填写0001、安道尔0376、巴哈马1242                                                                                                                                                      |
...
| receiverCity             | string      | 收货人地址的市                  | 建议          | 中文例如：北京市                                                                                                                                                                                                      |
| receiverDistrict         | string      | 收货人地址的区                  | 建议          | 中文例如：朝阳区                                                                                                                                                                                                      |
| receiverStreet           | string      | 收货人地址的街道                 | 可选参数        | 中文例如：望京街道                                                                                                                                                                                                     |
| receiverAddress          | string      | 收货人地址的详细地址               | 建议          | 中文                                                                                                                                                                                                            |
| deliverType              | string      | 配送方式                     | 建议          | 可选值：SELF_PICKUP：自提<br/>EXPRESS：快递                                                                                                                                                                             |
| groupOwnerId             | String      | 用户参与拼团的发起人的账号Id          | 建议          | 用户参与拼团的发起人的唯一标识                                                                                                                                                                                               |
...
| discounts                | json_array  | 券信息                      | 强烈建议        | 例如：<br/> `[{"type":"券","id":"xxx","activityId":"xxx", "amount":5.00},{"type":"下单立减","id":"","activityId":"xxx", "amount":10.00},{"type":"积分","id":"","activityId":"xxx", "amount":2.50}]`                     |
| item_price               | float64     | 订单实付价格                   | 必传参数        | 货币计量单位为中国大陆法定货币人民币 （CNY），单位：元 例如：88.69                                                                                                                                                                        |
| order_price              | float64     | 订单原价                     | 建议          | 货币计量单位为中国大陆法定货币人民币 （CNY），单位：元 例如：88.69                                                                                                                                                                        |
| sampleCabinetInfo        | json_object | 到柜领样场景相关的信息，详情见下文        | 必传参数        |                                                                                                                                                                                                               |
| sampleMachineInfo        | json_object | 派样机场景相关信息，详情见下文          | 可选参数        |                                                                                                                                                                                                               |
| receiverVilCode          | string      | 第三方地图API返回的具体小区代码        | 可选参数        | 如:"310115103205"                                                                                                                                                                                              |
| receiverAddressGpsInfo   | json_object | 收货地址GPS的相关信息，详情见下文       | 可选参数        |                                                                                                                                                                                                               |
...
receiverAddressGpsInfo的内容如下：

| **参数名称**   | **类型**    | **参数说明** | **是否必传** | **规范**                   |
|------------|-----------|----------|----------|--------------------------|
| longitude  | float64   | 收货地址经度   | 可选参数     | 如：118.567328，维持小数点后6位精度  |
| latitude   | float64   | 收货地址纬度   | 可选参数     | 如：34.112312，维持小数点后6位精度   |
...
| startCity          | string      | 参数的内容                     | 强烈建议     | 出发城市的名称或缩写                                                                       |
| endCity            | string      | 参数的内容                     | 强烈建议     | 到达城市的名称或缩写                                                                       |
| flightNum          | string      | 航班号                       | 必传参数     | 例：`9C3300`、`IJ123`，如果是中转联程或往返，把2段都写上，用`\|`分隔，例如：`9C3300\|9C1345`,`9C3300\|B3210` |
| branchId           | string      | 航空公司分支Id                  | 强烈建议     | 直接使用航司代码，如9C、IJ、IB，如果是联程，2段不一样的，用`\|`分隔，例如：`9C\|1B`                              |
| airlineId          | string      | 航线Id                      | 强烈建议     | 航线的机场三字码，如上海浦东机场-重庆江北机场，填：PVGCKG，若为中转联程或往返，把2段都写上，用`\|`分隔，例如：`PVGCKG\|CKGSHA`    |
...
|----------------|--------|----------|----------|---------------|
| [基础参数](#data)  |        |          |          | 需要满足基础参数的配置要求 |
| chatId         | string | 聊天会话id   | 强烈建议     |               |
| receiveTokenId | string | 主播的账号    | 必传字段     |               |
...
|----------------|--------|-----------|----------|-----------------------------|
| [基础参数](#data)  |        |           |          | 需要满足基础参数的配置要求               |
| chatId         | string | 聊天会话id    | 强烈建议     | 同一会话的发起聊天和结束聊天的chatId必须保持一致 |
| receiveTokenId | string | 主播的账号     | 必传字段     |                             |
| chatLength     | float  | 聊天时长，单位为秒 | 强烈建议     |                             |
...
| currency      | string | 计价货币     | 必传       | 参考货币代码表(ISO4217)<br/>[国际标准化组织 4217标准](https://baike.baidu.com/item/ISO%204217/11018231?fr=aladdin) |
...
```json
{
    ...
    "data": {
        ...
        "sampleCabinetInfo": [
            {
                "uploadPhotoState": 1,
                "VerificationCodeFirstTimestamp": 1465810229852
            }
        ]
    }
}
...
在某些场景下，为保障数据安全，我们需要对请求体进行加密处理后再进行传输。 若需开通加密传输方式，请联系数美科技客服人员。
本节将详细介绍如何实现这一过程，重点聚焦于使用SM4国密算法进行加密的实践方法。
